...
public class LockBasedListModTransporter implements Transporter {
    private final LinkedList<Token> list;
    private final Lock lock = new ReentrantLock();
    private final Condition notEmpty = lock.newCondition();

    public LockBasedListModTransporter(LinkedList<Token> list) {
        this.list = list;
    }
    ...
    public void push(Token token) throws InterruptedException {
        try {
            ...
            if (list.size() == 1) {
                // idea: signal only if list was empty
                notEmpty.signal();
            }
        } finally {
            lock.unlock();
