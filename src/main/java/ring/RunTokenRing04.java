...
import medium.buffer.BlockingQueueBufferedTokenMedium;
...
public class RunTokenRing04 {

    static final String name = "BlockingQueueBufferedTokenMedium";
                ...
                for (int i = 0; i < warmUpRuns + normalRuns; i++) {
                    ...
                    List<Medium> mediums = IntStream.range(0, rs)
                            ...
                            .map(j -> {
                                ...
                                BlockingQueue<Token> queue = new ArrayBlockingQueue<Token>(ibtn);
                                for (int k = 0; k < ibtn; k++) {
                                    Token token = new TokenImpl(destinationIndex, rs);
                                    queue.add(token);
                                }
                                return new BlockingQueueBufferedTokenMedium(queue);
                            })
